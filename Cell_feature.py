...
def main():
    ...
    if len(os.listdir(args.result_dir))<3:
        print("patch dir: ", args.patch_dir)
        print("json dir: ", args.json_dir)
        print("patch dir: ", args.result_dir)

        jsonAll = glob.glob(args.json_dir)
        for process_i,json_item in enumerate(jsonAll):
            slideName = json_item.split("/")[-3]

            slide_category = json_item.split("/")[-2]
            patchJson = glob.glob(json_item+"/*.json")
            print(f"{process_i} / {len(jsonAll)}    {slideName},    patch total:{len(patchJson)}")
            # ###创建特征保存文件
            # if not os.path.exists(args.result_dir):
            #     os.mkdir(args.result_dir)
            saveFile = os.path.join(args.result_dir, slide_category+"_"+slideName + ".csv")
